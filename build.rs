...
#[cfg(not(feature = "generate_protobuf_src"))]
fn guess_build_method() -> Result<Option<BuildMethod>> {
    Ok(None)
}
...
fn extract_src_file(src_file: impl AsRef<Path>) -> Result<PathBuf> {
...
fn compile_protobuf(dir: impl AsRef<Path>) -> Result<()> {
    let dir = dir.as_ref();
    let include_dir = dir;
    let proto_paths = {
